...
SL_WEAK void app_process_action(void)
{
  ...
  if(get_idx()>=DATASET_SIZE && done)
  {
      t1=sl_sleeptimer_get_tick_count();
      train(25);
      t2=sl_sleeptimer_get_tick_count();
      printf("Training Finished! Accuracy: %d %% | Training time: %f seconds\r\n", get_accuracy(),((float)(t2-t1))/sl_sleeptimer_get_timer_frequency());
      done=0;
  }
  //Collect DATASET_SIZE amount of data
  else if(done && get_idx()<DATASET_SIZE)
  {
    add_sample_to_data();
    done=0;
  }
